...
import { PageText, PageTitle, ViewDescription, ViewContainer, ViewProduct } from './style';
...
import Recipe from "@/components/Items/Recipes/index";
...
const PageDetailsProduct = () => {
  ...
  const productDataJson = Array.isArray(productDataParam) ? productDataParam[0] : productDataParam;
  ...
  React.useEffect(() => {
    ...
    const initialIngredientCosts = (product.productIngredients || []).reduce((acc, item) => {
      acc[item.id] = calculateItemCost(item.unitPriceSnapshot, item.quantity);
      return acc;
    }, {} as Record<number, number>);

    const initialServiceCosts = (product.productServices || []).reduce((acc, item) => {
      const cost = (item.unitPrice || 0) * (item.quantity || 1);
      acc[item.id] = cost;
      return acc;
    }, {} as Record<number, number>);

    const initialRecipeCosts = (product.productRecipes || []).reduce((acc, item) => {
  ...
  const totalCostValue = useMemo(() => {
    ...
    const recipesBaseCost = Object.values(recipeCosts).reduce((sum, cost) => sum + cost, 0);
